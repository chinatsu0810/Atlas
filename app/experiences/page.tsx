...
import { experienceTags, experiences, tags } from '@/lib/db/schema';
...
export default async function ExperiencesPage({
  ...
  if (selectedCountries.length > 0) {
    ...
    if (regularCountries.length > 0) {
      countryConditions.push(
        inArray(experiences.country, regularCountries)
      );
    }
  ...
  for (const categoryTagIds of selectedTagIdsByCategory.values()) {
    const matchingExperienceIds = db
      ...
      .from(experienceTags)
      .where(inArray(experienceTags.tagId, categoryTagIds));

    conditions.push(inArray(experiences.id, matchingExperienceIds));
  }
  ...
  const hasNextPage = experienceList.length > pageSize;
  const visibleExperiences = experienceList.slice(0, pageSize);
  ...
  const createPageUrl = (page: number) => {
    ...
    return `/experiences?${query.toString()}`;
  };
        ...
        <div className="mb-8 flex items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <Sparkles className="h-5 w-5 text-[#1478B8]" />
              <h1 className="text-xl font-bold">経験談を探す</h1>
            </div>

            <p className="mt-2 text-sm text-[#668096]">
              みんなのリアルな体験談から、知りたい情報を見つけましょう。
            </p>
          </div>
