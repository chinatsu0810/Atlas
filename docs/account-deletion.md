...
- 管理画面は `/account/users`（実装済み。下の「運営削除の画面」を参照）。
...
**設計・実装で決めるもの**
5. ~~運営削除の管理画面の場所と形~~: 決定済み（`/account/users`）。
...
| 運営削除の画面（`/account/users`、`/account/users/[id]`）と、再登録拒否の照合（サインアップ時） | **実装済み。** ログイン後の画面は、まだ目視していない（下記）。`deleteUser` と照合は、ロールバック付きで検証済み |
| プライバシーポリシー | 未着手 |

### 運営削除の画面

マイページの「運営」メニューに「ユーザー」を追加した。運営（`owner` ロール）だけが開ける（それ以外は「アクセスできません」）。

- **ユーザー一覧**（`/account/users`）: 削除されていない会員（新しい順、最大100人）と、削除の履歴（最近20件）。会員ごとに、ニックネーム・メールアドレス・登録日・公開中の投稿数を表示する。削除の履歴には、名前もメールアドレスも出ない（消去済みのため）。
- **ユーザー詳細**（`/account/users/[id]`）: 基本情報、削除の記録、削除フォーム。
  - 削除フォーム: 削除の種類（完全削除 / コンテンツを残して削除）、理由（必須・500文字まで）、再登録の拒否（チェック）、確認のための「削除」の入力。確認ダイアログも出す。
  - 運営アカウントと自分自身には、フォームを出さない（自分は、マイページの「ログイン情報」から削除する）。
  - 削除済みのユーザーは、削除の記録だけを表示する。
- 削除は、サーバーアクション `adminDeleteUser`（[app/account/users/actions.ts](../app/account/users/actions.ts)）が実行する。運営権限を確認し、入力を検証してから `deleteUser` を呼ぶ。失敗の理由は結果として返す。成功すると、一覧に戻って「削除しました」と表示する。
- 実装: [lib/account/admin-users.ts](../lib/account/admin-users.ts)（データ取得）、[lib/account/blocklist.ts](../lib/account/blocklist.ts)（再登録拒否の照合）。

**再登録拒否の照合**: サインアップ時に、メールアドレスの HMAC が `account_deletions.email_hash` にあれば、「このメールアドレスでは登録できません。心当たりがない場合は、お問い合わせください。」と表示して、登録を止める。大文字小文字と前後の空白は区別しない。

**確認状況**: ログインしていない状態では「アクセスできません」が表示されることと、ページがエラーなくコンパイルされることを確認した。運営としてログインした状態の表示と、実際の削除操作は、本番DBに記録が残るため、まだ実行していない。最初は、テスト用のアカウントを作って、「完全削除」を試すのがよい（記録は30日後にジョブが消す）。
...
テストデータの作成から検証まで、すべて1つのトランザクションの中で行い、最後に必ずロールバックする。接続先のDBに行は残らない（`serial` の採番は進む）。次を確認している（116項目）。

- 本人退会（完全削除）: 墓標化、削除の記録、関連テーブルの処理、他のユーザーに触れていないこと、二重実行の拒否
- 運営削除（コンテンツを残す・再登録拒否）: コンテンツが残ること、招待の扱い、メールのHMAC
- 再登録拒否の照合: 拒否した人のメール（大文字小文字・空白違いを含む）だけが拒否され、本人退会・拒否を選ばなかった運営削除・無関係のメールは拒否されないこと
