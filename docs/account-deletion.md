...
**実装は [app/privacy/page.tsx](../app/privacy/page.tsx)（`/privacy`）。** 以下は、書く内容を最初に整理したメモで、公開するページの文言はページ側が正。法的な文言は、公開前に専門家の確認を受けること。【 】は運用に合わせて決める箇所。
...
- **【訂正】調査の結果、現状は `activity_logs` にIPアドレスを保存していない**（`logActivity` の呼び出し側が、IPアドレスを渡していない。列には空文字が入る）。公開するポリシーには「現時点では、IPアドレスは保存していません」と書いた。IPアドレスの保存を始めるときは、ポリシーの文言と、削除時の処理（`ip_address` を NULL にする。`deleteUser` は対応済み）を確認すること。
...
| プライバシーポリシー（[app/privacy/page.tsx](../app/privacy/page.tsx)、`/about` からもリンク） | **たたき台を実装済み。未公開。** 公開の条件は「プライバシーポリシーの公開前チェック」を参照 |
| 利用規約（[app/terms/page.tsx](../app/terms/page.tsx)、`/about` からもリンク） | **たたき台を実装済み。未公開。** フッターのリンク（`/terms`）が、これで404でなくなる。公開の条件は「利用規約の公開前チェック」を参照 |
...
ポリシー（`/privacy`）は、「実際にそうなっていること」を書いている。書いてあることと実態が違うと、ポリシーが虚偽になる。公開するときは、次を満たす。

**公開の条件（実態を合わせる）**
1. **削除機能を、ポリシーと同時に（またはその前に）本番へ反映する。** 本人退会・パージ・お問い合わせの保持期間の削除は、まだローカルのコミットで、プッシュ・デプロイしていない。現在の本番の `deleteAccount` は、旧実装のまま。
2. **Vercelに `CRON_SECRET` を設定する。** 未設定だと日次ジョブが動かず、「30日後に完全に削除」「お問い合わせは1年後に削除」が実現しない。
3. **運営メールの削除（受信から1年）を、実際に運用する。** ポリシーに「運営者が削除します」と書いている。守れないなら、その文を消す。
4. **Neonのバックアップ「最大30日」を確認する。** Freeプラン（復元用履歴6時間）なら、ポリシーより短いので問題ない。有料プランで、30日を超えるスナップショットを設定していると、ポリシーが事実と合わなくなる。
5. **Google アナリティクスを使い続けること**（ポリシーに記載）。やめる場合は、ポリシーの記載も消す。

**コードで確認した、ポリシーの記載（変わったらポリシーも直す）**
- 取得する情報: ニックネーム・メールアドレス・パスワード（ハッシュ化）・投稿・お問い合わせ・操作履歴（IPアドレスは保存していない）・Cookie。
- 外部サービス: Vercel（配信）、Neon（DB。シンガポールリージョン）、Resend（お問い合わせの通知メール・パスワード再設定メール）、Google アナリティクス（`app/layout.tsx`）。**AI（Anthropic）とThreads連携は、運営の投稿案・会議・Threads分析だけを扱い、会員の情報を送っていない**ので、記載していない。課金（Stripe）は未稼働なので、記載していない。
- Cookie: ログイン用のセッションCookie（最後の利用から約24時間）と、Google アナリティクスのCookie。
- **会員は、自分の質問・回答・経験談を削除できない**（削除ボタンは運営にだけ表示され、サーバー側も運営のみ許可している）。会員が投稿を消す方法は、アカウントの削除か、お問い合わせフォームから運営に依頼するかの2つ。ポリシーと規約は、この実態に合わせてある。**会員が自分の投稿を削除できるようにしたら、ポリシー（3・9）と規約（第6条）を更新すること。**
- 課金を始める、AIに会員の投稿を渡す、新しい外部サービスを入れる、などのときは、ポリシーを更新する。

**運営が判断・確認するもの**
- 運営者の氏名（名称）・住所は、ページには載せず、「個人情報の開示請求（ポリシー9）または法令に基づく開示請求があった場合に、確認のうえ遅滞なくお知らせします。通常のお問い合わせには答えません」とした。ページに載せたい場合は、「10. 運営者について」に書く。**課金（Stripe）を始めるときは、特定商取引法の表記が別に必要になる**（請求を待つ形にするなら、その運用と表記の要件を、専門家に確認する）。
- 「Atlas運営」という名称でよいか。
- 制定日・最終更新日（`ENACTED_ON` / `UPDATED_ON`）を、公開日に合わせる。
- **法的な確認**: この文書は一般的な内容の草案。特に、電気通信事業法の「外部送信規律」（Google アナリティクスの記載・オプトアウトの案内が要件を満たすか）と、個人情報保護法上の公表事項は、専門家に確認してもらうのが望ましい。

**このポリシーが、まだカバーしていないもの**
- ポリシーの変更を、会員にどう知らせるか（現状は、ページの掲載と最終更新日の更新のみ）。

## 利用規約の公開前チェック

利用規約（`/terms`）も、ポリシーと同じく、実態に合わせて書いてある。公開するときは、次を確認する。

**運営が判断するもの（規約に書いた内容）**
- **投稿の利用許諾（第4条）**: 投稿は、当サービス上での表示・保存・検索・分類に加えて、「当サービスの紹介や宣伝のための、出典を明示した一部の引用（SNSでの紹介など）」を許諾する内容にした。SNS運用（Threads）で会員の投稿を引用する予定がなければ、この項目は削ってよい。引用する場合は、投稿者の同意を得る運用も検討する。
- **未成年（第2条）**: 「保護者の同意を得たうえでご利用ください」と書いた。実際には、年齢の確認はしていない。
- **現在は無料（第1条）**: 課金を始めるときは、規約を更新する（Stripeの扱い、返金、特定商取引法の表記など）。
- **管轄裁判所（第12条）**: 「運営の所在地を管轄する裁判所」とした。運営者の所在地を出したくない場合の書き方。
- **運営者の名称**: 「運営」と書いている。氏名（名称）・住所は、ポリシーと同じく、請求があれば開示する方針。

**実態との対応（変わったら規約も直す）**
- 会員は自分の投稿を削除できない（運営のみ）ため、第6条は「お問い合わせフォームから運営に依頼」と書いた。
- ニックネームの重複を認めていない（第2条）のは、`users_name_active_unique`（削除されていないユーザーの間でユニーク）に対応。
- 閲覧は登録なしでできる（第2条）のは、質問・経験談の一覧と詳細が、ログイン不要であることに対応。
- 運営がアカウントを削除できる（第5条）のは、`/account/users` の運営削除に対応。

**法的な確認**: この文書は一般的な内容の草案。免責の範囲（消費者契約法により、運営に故意・重大な過失がある場合まで責任を免れることはできない。第2・7・8条に「運営に故意または重大な過失がある場合を除いて」と書いた）と、投稿の利用許諾の範囲は、専門家に確認してもらうのが望ましい。

## デプロイの手順

ここまでの変更（本人退会、運営削除、パージ、お問い合わせの保持期間、ポリシー、規約）は、すべてローカルのコミットで、GitHub にプッシュしていない（`origin/main` より9コミット先）。本番に反映する手順は次のとおり。

**前提**: 本番も開発も、同じDB（Neon）に接続している。マイグレーション `0018`（`account_deletions`）は、すでに適用済み。追加のDB作業はない。

1. **Vercelの環境変数を確認する**（Production）。
   - `CRON_SECRET`: **新規に設定する**。十分に長いランダム文字列。未設定だと、日次ジョブは401を返して何もしない。
   - 既存の `POSTGRES_URL` / `AUTH_SECRET` / `RESEND_API_KEY` などは、そのまま。**`AUTH_SECRET` は再登録拒否のメールの識別子にも使う**ので、変更しないこと（変えると、保存済みの識別子と照合できなくなる）。
2. **プレビューで確認してからmainに反映する**（推奨）。`main` に直接プッシュすると、すぐ本番に反映される。
   - ブランチを作ってプッシュし、Vercelのプレビューのデプロイで、ビルドが通ることと画面を確認する。
   - ローカルでは、`tsc`（型チェック）は通っているが、`next build` は実行していない（開発サーバーの `.next` を壊さないため）。ビルドの確認は、プレビューで行う。
   - プレビューの環境変数（Preview）が未設定なら、設定するか、この手順は省いて、`main` にプッシュしてビルドの結果を確認する。
3. **反映後に確認する**。
   - `/privacy`、`/terms`、フッターのリンク、`/about` が表示される。
   - 運営としてログインして、マイページに「ユーザー」が出て、`/account/users` が開ける。
   - `/account/security` の削除確認画面に、新しい文言が出る。
   - 日次ジョブ: `curl -H "Authorization: Bearer <CRON_SECRET>" "https://www.atlas-community.jp/api/cron/purge-expired-data?dryRun=1"` が200を返し、対象件数が0件である。（Vercelのダッシュボードの Cron Jobs に、ジョブが登録されていることも確認する。）
4. **ためしに、テスト用のアカウントで退会してみる**（本番のDBに記録が残るが、30日後にジョブが消す）。
   - 新規登録して、質問を1件投稿し、マイページの「ログイン情報」から削除する。
   - 削除後、その質問が一覧・検索・詳細から消えること、ログインできないこと、`/account/users` の履歴に出ることを確認する。
5. **元に戻す方法**: 問題があれば、Vercelのダッシュボードで、直前のデプロイに戻せる（「Instant Rollback」）。DBの `account_deletions` は追加のみなので、コードを戻しても、既存の機能には影響しない。ただし、反映後に退会したユーザーのデータは、コードを戻しても元に戻らない。
