...
「今週の数字を分析する」
  ...
  → 分析と、分析時点の数字（スナップショット）をDBへ保存（threads_kpi_reports）

「今週分の10件を作成」（「直近のThreads分析を、テーマ・切り口の参考にする」にチェックがあるとき）
  → 保存済みの最新の分析 → lib/threads/analysis-note.ts で「参考情報」の文章にする
  → テーマ選定（thread-topic-planning）と、各投稿の企画（thread-planning）に渡す
```
...
- **分析の保存**: 分析のたびに `threads_kpi_reports` に保存する（分析の結果、指標の表、分析担当に渡した文脈、
  分析時点のスナップショット）。画面には保存済みの分析が新しい順に並び、過去の分析も見られる。保存に失敗しても、
  分析の結果は表示する（その場合は「保存されていません」と出る）。
- **前回の分析との比較**: 2回目以降の分析では、前回の分析を分析担当に渡す。こちらで計算して渡すのは、
  フォロワー数の増減、前回の分析に含まれていた投稿のうち今回も取得できたものの累計値の増加（同じ投稿の、前回の分析時点からの増加なので、
  経過日数をそろえた比較になる）、前回の分析以降に投稿された数。前回の分析がなければ、「今回が次回の比較の基準になる」と伝える。
  前回の分析の文章（傾向・仮説）は渡さない（前回の見立てに引きずられないため。数字だけを比べる）。
- **週次の投稿作成への反映**: 「今週分の10件を作成」では、保存済みの最新の分析を、テーマ選定と各投稿の企画に
  「参考情報」として渡せる（チェックボックス。分析があれば初期状態でオン）。運営が「分析」を実行して内容を確認し、
  「作成」を押すと、その分析が反映される。ただし次のように扱う。
  - サンプルが小さいため、**事実ではなく仮説**として渡す。「取り扱い」の注意を文章の冒頭に付ける。
  - テーマは分析に偏らせない。分析に沿ったテーマは、指定件数のうち多くても半分程度にとどめる（テーマ選定の指示）。
  - 分析の数字は、投稿の文面・採用理由に書かせない。
  - **ライター（執筆担当）には渡さない**。企画担当が方針に落とし込んだ内容だけが、ライターに届く。
  - 参考情報は、投稿案のレコードには保存しない（分析そのものは `threads_kpi_reports` に残る）。
...
- 分析結果はDBに保存するが、蓄積した知識としてAIが読む仕組み（Knowledge）は未実装。分析を投稿の反応データと結びつけた検証
  （「この仮説は当たっていたか」）は行っていない。
